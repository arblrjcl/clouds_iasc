...
module "route53_hosted_zones" {
  ...
  private_hosted_zone_name = "${var.project_environment_name}.${var.public_hosted_zone_name}"
  public_hosted_zone_tags  = merge(local.common_tags, { Name : "${var.public_hosted_zone_name}" })
  private_hosted_zone_tags = merge(local.common_tags, { Name : "${var.project_environment_name}.${var.public_hosted_zone_name}" })
