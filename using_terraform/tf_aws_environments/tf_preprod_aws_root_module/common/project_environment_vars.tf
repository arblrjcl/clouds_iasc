...
locals {
  ...
  vpc_ipv4_cidr_block                         = "10.255.0.0/16"
  vpc_tenancy                                 = "default"
  ...
  public_network_ipv4_cidr_blocks = [for index in range(0, local.number_of_availability_zones, 1) : "10.255.${10 + index}.0/24"]
  ...
  private_network_ipv4_cidr_blocks = [for index in range(0, local.number_of_availability_zones, 1) : "10.255.${50 + index}.0/24"]
