...
resource "civo_dns_domain_name" "custom_domain_name" {
...
resource "civo_network" "project_civo_network" {
  ...
  label          = "${var.project_name}-custom-network"
  nameservers_v4 = var.custom_v4_nameservers
  cidr_v4        = var.cidr_v4
}

resource "civo_firewall" "project_civo_custom_network_firewall" {
  #count                = var.default_network_instance ? 0 : 1
  name                 = "${var.project_name}-custom-network-firewall"
